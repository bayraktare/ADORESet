...
% Last Modified by GUIDE v2.5 16-Feb-2017 16:03:13
...
handles.folders = 'C:\Users\Ertugrul\Desktop\bottle\';
handles.path = dir(handles.folders);

global img_n obj_cl nti
img_n = 1; % index number of the current image
obj_cl = []; % category of the other objects not the main object
nti = 24; % number of total images within a single gui window
handles.num = img_n;
handles.PathImages = dir([handles.folders,'\*.jpg']);

ctg3={'muz','cilek','limon'}

for i=1:nti
    field1=['checkbox' num2str((i-1)*3+1)]
    field2=['checkbox' num2str((i-1)*3+2)]
    field3=['checkbox' num2str((i-1)*3+3)]
    field4=['radiobutton' num2str(i)]
    
    set(handles.(field1), 'String',ctg3{1});
    set(handles.(field2), 'String',ctg3{2});
    set(handles.(field3), 'String',ctg3{3});
end


handles = load_image(handles,handles.num);
...
function handles = load_image(handles,num)

global img_n nti

if img_n>size(handles.PathImages, 1)/nti
    msgbox('The END');
    return;
end
...
handles.images.p1 = imread([handles.folders,'\',handles.PathImages(1+(img_n-1)*nti).name]);
handles.images.p2 = imread([handles.folders,'\',handles.PathImages(2+(img_n-1)*nti).name]);
handles.images.p3 = imread([handles.folders,'\',handles.PathImages(3+(img_n-1)*nti).name]);
handles.images.p4 = imread([handles.folders,'\',handles.PathImages(4+(img_n-1)*nti).name]);
handles.images.p5 = imread([handles.folders,'\',handles.PathImages(5+(img_n-1)*nti).name]);
handles.images.p6 = imread([handles.folders,'\',handles.PathImages(6+(img_n-1)*nti).name]);
handles.images.p7 = imread([handles.folders,'\',handles.PathImages(7+(img_n-1)*nti).name]);
handles.images.p8 = imread([handles.folders,'\',handles.PathImages(8+(img_n-1)*nti).name]);
handles.images.p9 = imread([handles.folders,'\',handles.PathImages(9+(img_n-1)*nti).name]);
handles.images.p10 = imread([handles.folders,'\',handles.PathImages(10+(img_n-1)*nti).name]);
handles.images.p11 = imread([handles.folders,'\',handles.PathImages(11+(img_n-1)*nti).name]);
handles.images.p12 = imread([handles.folders,'\',handles.PathImages(12+(img_n-1)*nti).name]);
handles.images.p13 = imread([handles.folders,'\',handles.PathImages(13+(img_n-1)*nti).name]);
handles.images.p14 = imread([handles.folders,'\',handles.PathImages(14+(img_n-1)*nti).name]);
handles.images.p15 = imread([handles.folders,'\',handles.PathImages(15+(img_n-1)*nti).name]);
handles.images.p16 = imread([handles.folders,'\',handles.PathImages(16+(img_n-1)*nti).name]);
handles.images.p17 = imread([handles.folders,'\',handles.PathImages(17+(img_n-1)*nti).name]);
handles.images.p18 = imread([handles.folders,'\',handles.PathImages(18+(img_n-1)*nti).name]);
handles.images.p19 = imread([handles.folders,'\',handles.PathImages(19+(img_n-1)*nti).name]);
handles.images.p20 = imread([handles.folders,'\',handles.PathImages(20+(img_n-1)*nti).name]);
handles.images.p21 = imread([handles.folders,'\',handles.PathImages(21+(img_n-1)*nti).name]);
handles.images.p22 = imread([handles.folders,'\',handles.PathImages(22+(img_n-1)*nti).name]);
handles.images.p23 = imread([handles.folders,'\',handles.PathImages(23+(img_n-1)*nti).name]);
handles.images.p24 = imread([handles.folders,'\',handles.PathImages(24+(img_n-1)*nti).name]);
% handles.images.p25 = imread([handles.folders,'\',handles.PathImages(20+(img_n-1)*20).name]);
% handles.images.p26 = imread([handles.folders,'\',handles.PathImages(20+(img_n-1)*20).name]);
% handles.images.p27 = imread([handles.folders,'\',handles.PathImages(20+(img_n-1)*20).name]);
% handles.images.p28 = imread([handles.folders,'\',handles.PathImages(20+(img_n-1)*20).name]);
% handles.images.p29 = imread([handles.folders,'\',handles.PathImages(20+(img_n-1)*20).name]);
% handles.images.p30 = imread([handles.folders,'\',handles.PathImages(20+(img_n-1)*20).name]);


% set(handles.axes1, 'Xlim', [1,size(handles.images.c1,2)], 'YLim', [1,size(handles.images.c1,1)]);
imshow(handles.images.p1,'Parent', handles.axes1);
imshow(handles.images.p2,'Parent', handles.axes2);
imshow(handles.images.p3,'Parent', handles.axes3);
imshow(handles.images.p4,'Parent', handles.axes4);
imshow(handles.images.p5,'Parent', handles.axes5);
imshow(handles.images.p6,'Parent', handles.axes6);
imshow(handles.images.p7,'Parent', handles.axes7);
imshow(handles.images.p8,'Parent', handles.axes8);
imshow(handles.images.p9,'Parent', handles.axes9);
imshow(handles.images.p10,'Parent', handles.axes10);
imshow(handles.images.p11,'Parent', handles.axes11);
imshow(handles.images.p12,'Parent', handles.axes12);
imshow(handles.images.p13,'Parent', handles.axes13);
imshow(handles.images.p14,'Parent', handles.axes14);
imshow(handles.images.p15,'Parent', handles.axes15);
imshow(handles.images.p16,'Parent', handles.axes16);
imshow(handles.images.p17,'Parent', handles.axes17);
imshow(handles.images.p18,'Parent', handles.axes18);
imshow(handles.images.p19,'Parent', handles.axes19);
imshow(handles.images.p20,'Parent', handles.axes20);
imshow(handles.images.p21,'Parent', handles.axes21);
imshow(handles.images.p22,'Parent', handles.axes22);
imshow(handles.images.p23,'Parent', handles.axes23);
imshow(handles.images.p24,'Parent', handles.axes24);
% imshow(handles.images.p25,'Parent', handles.axes25);
% imshow(handles.images.p26,'Parent', handles.axes26);
% imshow(handles.images.p27,'Parent', handles.axes27);
% imshow(handles.images.p28,'Parent', handles.axes28);
% imshow(handles.images.p29,'Parent', handles.axes29);
% imshow(handles.images.p30,'Parent', handles.axes30);
...
set(handles.text1,'String',[num2str(img_n),'/',num2str(floor(size(handles.PathImages, 1)/nti))])

for i=1:nti

    
    feature{i,1}=handles.PathImages(i+(img_n-1)*nti).name
    
    field1=['checkbox' num2str((i-1)*3+1)]
    field2=['checkbox' num2str((i-1)*3+2)]
    field3=['checkbox' num2str((i-1)*3+3)]
    field4=['radiobutton' num2str(i)]
    if get(handles.(field1), 'Value')==1
       feature{i,2} = get(handles.(field1), 'String');
    end
    if get(handles.(field2), 'Value')==1
       feature{i,3} = get(handles.(field2), 'String');
    end
    if get(handles.(field3), 'Value')==1
       feature{i,4} = get(handles.(field3), 'String');
    end
     if get(handles.(field4), 'Value')==1
       movefile([handles.folders,'\',handles.PathImages(i+(img_n-1)*nti).name],[handles.folders,'\deleted\'])
     else
         movefile([handles.folders,'\',handles.PathImages(i+(img_n-1)*nti).name],[handles.folders,'\approved\'])
    end
    feature{i,5}=get(handles.edit1, 'String')
    
end

ft=cell2table(feature)

writetable(ft, 'sonuctemp.txt','Delimiter','\t')
system('type sonuctemp.txt sonuc.txt > sonuc.txt')
...
for i=1:nti
    ...
    set(handles.(field1), 'Value',0);
    set(handles.(field2), 'Value',0);
    set(handles.(field3), 'Value',0);
    set(handles.(field4), 'Value',0);
end

img_n = img_n+1;
load_image(handles,handles.num);
